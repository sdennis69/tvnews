          ...
          <Link key={post.id} href={`/article/${post.slug}`}>
            <a className="flex gap-3 bg-white rounded-lg overflow-hidden hover:shadow-lg border border-[#CCCCCC] transition-all duration-200 group">
              ...
              <div className="flex-1 p-3 flex flex-col justify-between">
                <div>
                  <span className="bg-[#003D7A] text-white text-xs font-bold px-2 py-1 rounded inline-block mb-1">
                    {category.toUpperCase()}
                  </span>
                  <h3 className="text-sm font-bold text-[#333333] group-hover:text-[#003D7A] transition-colors line-clamp-2 leading-tight">
                    {post.title}
                  </h3>
                </div>
