/**
 * /national-world-news — National/World News category listing page
 * Layout: horizontal thumbnail-left cards matching Local News page style
...
async function fetchMorePosts(after: string): Promise<{ posts: Post[]; endCursor: string | null; hasNextPage: boolean }> {
  const query = `
    query GetMoreNationalWorld($after: String) {
      posts(first: ${PAGE_SIZE}, after: $after, where: { categoryName: "national-world-news" }) {
        edges { node {
...
export default function NationalWorldNewsPage({ posts: initialPosts, initialEndCursor, initialHasNextPage, navItems }: Props) {
      ...
      <main className="min-h-screen bg-[#F5F5F5]">
        ...
        <div className="bg-[#003D7A] py-6 border-b-4 border-[#CC0000]">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h1 className="text-3xl font-bold text-white uppercase tracking-wide">National / World News</h1>
            <p className="text-blue-200 text-sm mt-1">Stories from across the nation and around the globe</p>
          </div>
        </div>
          ...
          <div style={{ display: 'flex', gap: '2rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: '1 1 500px', minWidth: 0 }}>
                      ...
                      <div style={{ flex: 1, padding: '1rem', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                        <div>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '0.5rem' }}>
                            <span className="bg-[#003D7A] text-white text-xs font-bold px-2.5 py-1 rounded">NATIONAL/WORLD</span>
              ...
              <div className="border-b-2 border-[#003D7A] mb-4">
                <span className="bg-[#003D7A] text-white text-xs font-bold px-3 py-1 uppercase tracking-wider inline-block">Latest National &amp; World Stories</span>
              </div>

              {/* Horizontal card list */}
              <div className="space-y-4">
                {rest.map((post) => {
                  const category = post.categories?.edges?.[0]?.node?.name || 'NATIONAL'
              ...
              {posts.length === 0 && (
                <div className="text-center py-16 text-[#666666]">
                  <p>No national/world stories found. Check back soon.</p>
                </div>
              )}
...
export const getStaticProps: GetStaticProps = async () => {
  try {
    const result = await getPostsByCategory('national-world-news', PAGE_SIZE)
    ...
    console.error('Error fetching national/world news:', err)
